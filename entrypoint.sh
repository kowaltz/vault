...
if [ "$1" = 'server' ]; then
    ...
    if [[ -z "${NODE_ID}" ]]; then
        echo "Error: Please set NODE_ID."
        exit 1
    elif [ "$NODE_TYPE" != "child" ] && [ "$NODE_TYPE" != "root" ]; then
        echo "Error: Please set NODE_TYPE. Accepted values are \"child\" and \"root\"."
        exit 1
    fi

    source $VAULT_CONFIG_DIR/create.sh
    config $NODE_ID
